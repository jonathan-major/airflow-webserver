...
def init_role(sm, role_name, role_vms, role_perms):
    ...
    if invalid_perms:
        raise Exception('The following permissions are not valid: {}'.format(invalid_perms))
    invalid_vms = [v for v in role_vms if v not in valid_vms]
    if invalid_vms:
        raise Exception('The following view menus are not valid: {}'.format(invalid_vms))
